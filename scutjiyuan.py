...
#设置主页
initial_page = 'http://cs.scut.edu.cn/xygk/kytz/'
#改变超链接筛选规则
rules = '//li/a[@target="_blank"]/@href'
    ...
    while(True): 
        if url_queue.qsize()>0:
            ...
            if re.search(pattern1,text):
                ...
                get=0
                insertlist =  ['','','','','','','',0,0]
                textlist = re.findall(r"(?<=题目\s:).*?(?=\n)|(?<=报告题目1：).*?(?=\n)" ,text)
                if len(textlist)>0:
                    #print textlist[0]
                    insertlist[0]=textlist[0]
                    get=1
                textlist = re.findall(r"(?<=报告人\s：).*?(?=\n)|(?<=Speaker:).*?(?=\n)|(?<=报\s告\s人：).*?(?=\n)",text)
                if len(textlist)>0:
                    #print textlist[0]
                    insertlist[1]=textlist[0]
                    get=1
                #stime
                textlist = re.findall(r"(?<=时间\s：).*?(?=\n)|(?<=间：).*?(?=\n)",text)
                if len(textlist)>0:
                    #print textlist[0]
                    ...
                    get=1
                textlist = re.findall(r"(?<=地点\s：).*?(?=\n)|(?<=点：).*?(?=\n)",text)
                if len(textlist)>0:
                    #print textlist[0]
                    insertlist[4]=textlist[0]
                    get=1
                textlist = re.findall(r"(?<=日期：).*?(?=\n)",text)
                if len(textlist)>0:
                    #print textlist[0]
            ...
            link_list = map(lambda i: 'http://cs.scut.edu.cn' + i if i[0] == '/' else 'http://cs.scut.edu.cn/' + i if i[0] == '#' else 'http://cs.scut.edu.cn/' + i if i[0]<>'h' else i, link_list)
